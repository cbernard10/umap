...
function RealGraph(graph){
    ...
    this.computeFuzzySimplices = () => {
        this.graph.edges = new Set()
        for(vi of Object.keys(this.graph.vertices)){
            if(this.graph.vertices[vi].neighbors[0]){
                ...
                for(vj_id of Object.keys(this.graph.vertices[vi].neighbors)){
                    ...
                    this.graph.vertices[vi].neighbors[vj_id]['weight'] = w_ij
                }
            }
        }
        for(vi of Object.keys(this.graph.vertices)){
            if(this.graph.vertices[vi].neighbors[0]){
                for(vj_id of Object.keys(this.graph.vertices[vi].neighbors)){
                    const vj = this.graph.vertices[vi].neighbors[vj_id].xi
                    if(vi === vj) continue
                    const w_ij = this.graph.vertices[vi].neighbors[vj_id].weight
                    const w_ji = this.graph.vertices[vj].neighbors.filter(n => n.xi == vi).weight
                    console.log(vi, vj, w_ij, w_ji)
    ...
    this.compute_sigma = d => {
        const k = d.length
        const rho_i = d[0]
        const sigmas = [...Array(10000).keys()].map(e => e)
